	...
	public void start(Stage primaryStage) throws Exception{
		...
		for (i = 0; i <= 8; i++){//Draw one face:
			...
			//Draw eyes:
			 ...
			 gc.fillOval(X_LEFT_EYE0 + xOffset, Y_LEFT_EYE0 + yOffset,EYE_WIDTH, EYE_HEIGHT);//Draw nose:
			 gc.setFill(Color.BLACK);
			 gc.fillOval(X_NOSE0 + xOffset, Y_NOSE0 + yOffset,NOSE_DIAMETER, NOSE_DIAMETER);//Draw mouth:
			 gc.setStroke(Color.RED);
			 gc.strokeArc(X_MOUTH0 + xOffset, Y_MOUTH0 + yOffset,MOUTH_WIDTH, MOUTH_HEIGHT0 +i + 1,MOUTH_START_ANGLE, MOUTH_EXTENT_ANGLE,ArcType.OPEN);
			 gc.setStroke(Color.BLACK);
			 gc.setFont(Font.font("Times New Roman", 12));
			gc.fillText("Kiss!!.",X_FACE0 + xOffset + FACE_DIAMETER+8, Y_FACE0 + yOffset+8);
		}
		xOffset = 50 * i;
		yOffset = 30 * i;
		//Draw kissing face:
		...
		gc.fillOval(X_MOUTH0 + xOffset + 10, Y_MOUTH0 + yOffset,MOUTH_WIDTH - 20, MOUTH_WIDTH - 20);//Add text in Times New Roman, 12 point:
		gc.setFill(Color.BLACK);
		gc.setFont(Font.font("Times New Roman", 12));
		...
		gc.setFill(Color.BLACK);
		gc.setStroke(Color.BLACK);
		gc.strokeOval(X_FACE0 + xOffset, Y_FACE0 + yOffset,FACE_DIAMETER, FACE_DIAMETER);//Draw eyes:
		gc.setFill(Color.BLUE);
		gc.fillOval(X_RIGHT_EYE0 + xOffset, Y_RIGHT_EYE0 + yOffset,EYE_WIDTH, EYE_HEIGHT);
		gc.fillOval(X_LEFT_EYE0 + xOffset, Y_LEFT_EYE0 + yOffset,EYE_WIDTH, EYE_HEIGHT);//Draw nose:
		gc.setFill(Color.BLACK);
		...
		gc.setFill(Color.BLACK);
		gc.setStroke(Color.RED);
		gc.strokeArc(X_MOUTH0 + xOffset, Y_MOUTH0 + yOffset, MOUTH_WIDTH,MOUTH_HEIGHT0 +  (i + 2),MOUTH_START_ANGLE, MOUTH_EXTENT_ANGLE, ArcType.OPEN);//Add text in Courier New Font, 16 point:
		gc.setStroke(Color.BLACK);
